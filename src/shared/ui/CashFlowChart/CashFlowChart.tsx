...
export function CashFlowChart({ points, selectedPeriod, onSelectPeriod, testID }: CashFlowChartProps) {
    ...
    <View style={styles.container} testID={testID}>
      ...
      >
        <Svg width={width} height={CHART_HEIGHT}>
          {ticks.map((tick) => {
            const y = scaleValueToY(tick, domain, plotHeight);
            return (
              <G key={tick}>
                <Line
                  x1={AXIS_LABEL_WIDTH}
          ...
          <Path d={buildBandPath(p80Coords, p20Coords)} fill={theme.colors.brand} fillOpacity={0.15} />
          ...
          <Path d={buildLinePath(confirmedCoords)} stroke={theme.colors.success} strokeWidth={2} fill="none" />

          <Path d={buildLinePath(p50Coords)} stroke={theme.colors.brand} strokeWidth={2} fill="none" />
          ...
          {points.map((point, index) => {
            ...
            return (
              <SvgText
                ...
                y={plotHeight + MONTH_LABEL_HEIGHT - 4}
                fontSize={theme.typography.bodySmall.fontSize}
                fill={theme.colors.textSecondary}
                textAnchor="middle"
              >
          ...
          {/*
            달마다 하나씩 두는 선택 터치 영역. 12칸을 한 화면 너비에 나누다
            보니 칸 하나의 폭이 44pt 접근성 기준보다 작을 수 있다 — 세로는
            plotHeight 전체라 44pt를 넘지만, 가로까지 만족시키려면 차트
            자체가 비현실적으로 넓어진다. 아래 표의 각 행(Pressable)이
            44pt를 만족하는 대체 선택 수단이다.
          */}
          {onSelectPeriod
            ? points.map((point, index) => {
                const { left, right } = monthSlotBounds(index, count, plotWidth);
