...
API 계약의 기준은 `docs/api/openapi-1.7.0.json`(OpenAPI 1.7.0, 배포된 백엔드의 현재 버전)이다. 백엔드 실행·화면별 호출 순서와 오류 처리는 [백엔드 연동 가이드](docs/backend-integration.md)에서 확인하고, 빠른 계약 요약은 [모바일·백엔드 API 계약](docs/integration.md)을 참고한다.
...
- 시나리오 선택(S03)의 "직접 입력"은 합성 거래 CSV 업로드 화면으로 이어진다. AI 분류 확인이 필요하면 거래별 카테고리를 확인한 뒤 서버가 새로 발급한 `dataset_id`로 분석한다.
...
3. **시나리오 선택** (`/scenario`) — 직접 입력 또는 데모 3개 중 선택(scenario_id·기본값은 배포된 백엔드의 `GET /v1/demo-scenarios` 응답과 맞춘 것이다, 2026-09-20 확인):
   ...
   데모를 선택하면 이후 입력 화면에 해당 시나리오의 기본값이 미리 채워진다. 직접 입력은 모든 값을 빈 상태로 시작하며, 해당 사항이 없어도 0 또는 아니오를 명시해야 한다. api 모드의 직접 입력은 합성 거래 CSV 등록(`/dataset`)을 먼저 거쳐 `dataset_id`를 확보한다.
